...
//
    ...
    public function index()
    {
        ...
        $this->load->view('pedidos/basic_index', $data);
    ...
    public function nuevo()
    {
        ...
        $data['consecutivo'] = $this->pedidos_model->maximoConsecutivo();
        $data['vendedores'] = $this->db->where('tipo_empleado_id',20)->get('empleados')->result();
    	$this->accesos_model->menuGeneral();
    ...
    {
    	$data['nota'] = $this->pedidos_model->select($idNota);
    ...
    public function guardarNota()
    {
                    ...
                    if($dato_prod->prod_tipo==1){
                        if($_POST['notaId']==''){
                            if($cantidad[$i]==0 OR $cantidad[$i]>$prod_stock){
                             ...
                             $item_producto = $this->db->get('pedido_detalle')->row();
    	...
    	$result = $this->pedidos_model->guardarNota();
    ...
    {
    	$rsDatos = $this->pedidos_model->getMainList();
    	sendJsonData($rsDatos);
    }
    public function getMainListDetail()
    {
        $rsDatos = $this->pedidos_model->getMainListDetail();
        sendJsonData($rsDatos);        
    }
    public function decargarPdf($idNota)
    {
        $rsNota = $this->db->from("pedido as np")
        ...
        $html = $this->load->view("templates/pedido.php",$data,true);                   
        /*escribimos archivo*/
    ...
    public function eliminar_nota($id){
        ...
        $this->db->where('notap_id',$id);
        $nt = $this->db->get('pedido')->row();

        $this->db->where('notapd_notap_id',$id);
        $adp = $this->db->get('pedido_detalle')->row();
    ...
    public function exportarExcel()
    {
        ...
        $result = $this->db->from("pedido_detalle i")
                            ->join("pedido com","com.notap_id=i.notapd_notap_id")
        ...
        $filename = 'pedido---' . date("d-m-Y") . '---' . rand(1000, 9999) . '.xls'; //save our workbook as this file name
