...
class Comprobantes_orden_compras_model extends CI_Model {
    ...
    public function select($id = '',$serie = '',$numero = '',$fecha_de_emision = '',$fecha_de_vencimiento = '',$cliente_id = '',$tipo_documento_id = '',$com_adjuntos = '',$anulado = '',$inicio = FALSE,$limite = FALSE, $empresa_id = '',$numero_pedido='',$numero_guia='',$orden_compra=''){
        if($id != ''){
            $sql = "SELECT "
                    . " com.incluye_igv, "
                    . " com.fecha_de_emision fecha_sunat,"
                    . " com.tipo_pago_id as pago_id, "
                    . " DATE_FORMAT(fecha_de_emision, '%d-%m-%Y') AS fecha_de_emision, "
                    . " epr.ruc empresa_ruc, "
                    . " epr.empresa empresa, "
                   ...
                   // . " tdc.abr abr, "
                    ...
                    . " mon.simbolo simbolo"
                    . " FROM comprobantes_orden_compras com "
...
$sql = "SELECT COUNT(com.id) as comprobante_id "
    ...
    public function insertar($data) {
        $rs = $this->db->insert('comprobantes_orden_compras', $data);
        if(!$rs){
    ...
    public function modificar($data, $where){
        ...
        $this->db->update('comprobantes_orden_compras', $data);
    ...
    public function eliminar($comprobante_id){
        ...
        $com = $this->db->get('comprobantes_orden_compras')->row();

        $this->db->where('comprobante_orden_id',$comprobante_id);
        $items = $this->db->get('items_orden_compras')->result();
        
        

        $this->db->where('id',$comprobante_id);
        $this->db->delete('comprobantes_orden_compras');
    ...
    public function getMainList() {
        ...
        $select = $this->db->select('emp.ruc as ruc, 
                                    comp.id as comp_id,
                                    cli.prov_razon_social as cliente,
                                    comp.total_gravada as total_gravada,
                                    comp.fecha_de_emision as fecha_de_emision,
                                    comp.cliente_id as cliente_id, 
                                    comp.anulado as anulado,comp.serie as serie,
                                    comp.numero as numero,
                                    comp.total_igv as total_igv,
                                    comp.total_a_pagar,CONCAT(e.nombre," ",e.apellido_paterno) as vendedor',false)
                           ->from("comprobantes_orden_compras comp")
   ...
   //////// LE //////////
    ...
    public function getCompras($where){
      ...
      $this->db->from('comprobantes_orden_compras com');
