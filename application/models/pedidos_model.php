...
class Pedidos_model extends CI_Model
{
    ...
    {
        $rsNota = $this->db->select("np.*,cli.*,em.nombre as vendedor_nombre,em.apellido_paterno as vendedor_apellido")
                           ->from("pedido as np")
        ...
        $rsDetalles = $this->db->from("pedido_detalle")
    ...
    public function guardarNota()
    {
        ...
        if($_POST['notaId'] == '')
        {
            ...
            $this->db->insert("pedido", $dataInsert);
            ...
            $this->db->update("pedido", $dataUpdate);
            $idNota = $_POST['notaId'];       
        ...
        $rsDetalle = $this->db->from("pedido_detalle")
                              ->where("notapd_notap_id", $idNota)
                              ->get()
                              ->result();
        if(count($rsDetalle)>0)
        {
            ...
            $this->db->delete("pedido_detalle",["notapd_notap_id"=>$idNota]);
        }
        ...
        for($i=0;$i<$cantidadIngresos;$i++)
        {
            ...
            $this->db->insert("pedido_detalle", $dataInsertDetalle);
    ...
    {
        //obtenemos el maximo consecutivo del las pedido
        $select = $this->db->from("pedido")
    ...
    {
        $est = array(ST_NOTA_ACTIVA,ST_NOTA_USADA);
        ...
        $select = $this->db->select('nota.*,CONCAT(em.nombre," ",em.apellido_paterno) as empleado,cli.*,mon.*',FALSE) 
                           ...
                           ->where_in("nota.notap_estado",$est );

       if($this->session->userdata('tipo_empleado_id')==20){
          $select->where("nota.notap_vendedor", $this->session->userdata('empleado_id'));
       }

                      $select->order_by("nota.notap_id", "desc");
       ...
       if($_POST['cliente_search'] > 0)
        ...
        if ($_POST['vendedor']!='') {
            $select->where('nota.notap_vendedor',$_POST['vendedor']);
        }
        /*obtener el total*/
        $count = clone $select;
        ...
        $query = $select->get();
        $rspedido = $query->result();  

        foreach($rspedido as $nota)
        {
            if($nota->notap_estado==1){
              $nota->estado = 'PENDIENTE';
            }else if($nota->notap_estado==3){
            ...
            $nota->boton_pdf = '<a href="'.base_url().'index.php/pedidos/decargarPdf/'.$nota->notap_id.'" target="_seld" ><img src="'.base_url().'/images/pdf.png" data-id="'.$nota->notap_id.'" class="descargar-pdf"></a>';
        }      
    ...
    public function getMainListDetail()
    {
        ...
        $select = $this->db->from("pedido_detalle")
