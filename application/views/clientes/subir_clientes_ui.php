...
<script>
 	$(document).ready(function(){
      $('#msj-error').hide();
	    $("#files").kendoUpload({
	        async: {
	            saveUrl: "<?php echo base_url()?>index.php/clientes/guardarSubidaClientes",
	            autoUpload: true,
	        },
          multiple:false,
          validation: {
          ...
          progress:function(){
	        ...
	        success:function(){
	           //dataSource.read();
             toast("success", 1500, "clientes importados!");
             location.href = '<?PHP echo base_url()?>index.php/clientes/index'; 
	        },
	        error:function(e){
            toast("error", 1500, "No se pudo Importar clientes");
            $('#msj-error').show();
