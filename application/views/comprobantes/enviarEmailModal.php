          ...
          <form id="formEnviarEmail">
            <input type="hidden" name="comprobante_id" id="comprobante_id" value="<?= $comprobante['comprobante_id']?>">
              <div class="col-xs-12 col-md-12 col-lg-12">
                <label>Correo
                <input class="form-control col-xs-12 col-md-12 col-lg-12" type="text" name="correo" id="correo" value="<?= $comprobante['cli_email']?>">
            </label>
          </div>
          </form>
	       </div>        
...
$(document).ready(function(){
  $(document).on("click",".enviarEmail",function(){
    ...
    $.ajax({
      url: '<?= base_url()?>index.php/comprobantes/enviarEmailModal_g',
      method: 'POST',
      dataType: 'JSON',
