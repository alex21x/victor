          ...
          <form id="formEnviarEmail">
            <input type="hidden" name="historia_id" id="historia_id" value="<?= $historia->his_id?>">
              <div class="col-xs-12 col-md-6 col-lg-6">
                <label>Correo
                    <input class="form-control col-xs-12 col-md-12 col-lg-12" type="text" name="correo" id="correo" value="<?= $historia->pac_correo?>">
                </label>
          </div>
          </form>
	       </div>        
...
$(document).ready(function(){
  $(document).on("click",".enviarEmail",function(){
    ...
    $.ajax({
      url: '<?= base_url()?>index.php/historias/enviarEmailModal_g',
      method: 'POST',
      dataType: 'JSON',
