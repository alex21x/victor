		...
		<form id="formPago">
			...
			<div class="row rowCobro">
				...
				<div class="col-xs-2 col-md-2 col-lg-2">
					<button id="btn_guardar_pago" type="button" class="btn btn-primary">Agregar Pago</button>
				</div>		
				<div class="col-xs-2 col-md-2 col-lg-2 col-md-offset-1">
					<a id="pdfPago" href="#"><img src="<?= base_url();?>images/pdf.png"></a>
                    <a id="ticketPago" href="#"><span class="glyphicon glyphicon-print"></span></a>
				</div>          
		</form>	<br><br>		
		<div id="rowPago"></div>				
	</div>
...
<script type="text/javascript">	
	...
	function rowPago(){
		$.ajax({
			...
			url: '<?= base_url()?>index.php/pagos/rowPago',
			method: 'POST',
			dataType: 'HTML',
			data: $("#formPago").serialize(),
			success: function(response){
					$("#rowPago").html(response);					
				}							
		});
	}
	...
	$("#btn_guardar_pago").click(function(){	
		...
		$.ajax({
			...
			url: '<?= base_url()?>index.php/pagos/guardar',
			method: 'POST',
			dataType: 'JSON',
			data: $("#formPago").serialize(),
			success: function(response){
				...
				if(response.status == STATUS_OK){					
					...
					toast("success",1500,'Pago registrado correctamente');
					//$("#myModal").modal('hide');					
				}				
			}
		});
	...
	$(document).on("click",".removePago",function(e){		
		e.preventDefault(); 
		var idPago = $(this).data('id');              
		var monto = $(this).data('monto');
        var msg = "Está seguro de eliminar Pago?";
        var url = '<?php echo base_url()?>index.php/pagos/eliminar/'+idPago
                            ...
                            action:function(){
                                $.ajax({
                                    ...
                                    success:function(response){
                                        if(response.status == STATUS_OK)
                                        {
                                            toast('success', 1500, 'Pago eliminado');
                                            rowPago();
                                            listaComprobantes();
                                        ...
                                        if(response.status == STATUS_FAIL)
                                        {
                                            toast('error', 1500 ,'No se puedo eliminar Pago.');
                                        }
                                    }
                                });
                            }
...
function listaComprobantes(){
		...
		$.ajax({
			...
			url: '<?= base_url()?>index.php/pagos/listaComprobantes',
			method: 'POST',
			dataType: 'HTML',
			data: $("#formPago").serialize(),
			success: function(response){
	...
	$("#agregarPago").click(function(){
        ...
        url: '<?= base_url()?>index.php/pagos/modal_crearr',
			method: 'POST',
			dataType: 'JSON',
			data: $("#formPago").serialize(),
            success: function(response){
	...
	//PDF
    $(document).on("click","#pdfPago",function(){
        ...
        javascript:window.open('<?= base_url()?>index.php/pagos/pdfPago/'+tipoComprobante+'/'+comprobante_id+'/'+cliente_id+'/'+vendedor_id+'/'+serNum+'/'+totalCredito,'','width=750,height=600,scrollbars=yes,resizable=yes');
    });

    //IMPRIMIR TICKET
    var PrintWindow;
    $(document).on("click","#ticketPago",function(){
        var cliente_id = $("#cliente_id_m").val();
        var vendedor_id = $("#vendedor_id_m").val();
    	var comprobante_id = $("#comprobante_id").val();
    	var tipoComprobante = $("#tipoComprobante").val();
        ...
        PrintWindow = window.open('<?= base_url()?>index.php/pagos/ticketPagopdf/'+tipoComprobante+'/'+comprobante_id+'/'+cliente_id+'/'+vendedor_id+'/'+serNum+'/'+totalCredito,'','width=750,height=600,scrollbars=yes,resizable=yes');
    });
});
