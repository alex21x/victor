  ...
  <div class="modal-dialog  modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">ENVIAR PROFORMA</h5>
          ...
          <form id="formEnviarEmail">
            <input type="hidden" name="proforma_id" id="proforma_id" value="<?= $proforma->prof_id?>">
              <div class="col-xs-12 col-md-12 col-lg-12">
                <label>Correo
                <input class="form-control col-xs-12 col-md-12 col-lg-12" type="text" name="correo" id="correo" value="<?= $proforma->email?>">
            </label>
          </div>
          </form>
	       </div>        
...
$(document).ready(function(){
  $(document).on("click",".enviarEmail",function(){
    ...
    $.ajax({
      url: '<?= base_url()?>index.php/proformas/enviarEmailModal_g',
      method: 'POST',
      dataType: 'JSON',
  ...
  $(document).on("click",".btn_cerrar",function(){
    ...
    setTimeout(function() { 
      location.href='<?PHP echo base_url()?>index.php/proformas/index/'+proforma_id;
       }, 2);
  });
 }); 
</script>
