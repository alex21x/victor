...
$data['medida'] = $this->medida_model->select();
   ...
   //print_r($rsDetalle);exit();
    ...
    public function create_pdf($comprobante_id='')
    {
        ...
        //var_dump($data);exit;
        $html = $this->load->view("templates/invoice_email.php",$data,true);
        ...
        fclose($file);
        //convertimos el html en pdf
        exec('"././wkhtmltox/bin/wkhtmltopdf" '.$rutaArchivoHtml.' '.$rutaArchivoPdf);
        //exec('"././wk/bin/wkhtmltopdf" '.$rutaArchivoHtml.' '.$rutaArchivoPdf);
        unlink($rutaArchivoHtml);
        //unlink($rutaArchivoPdf);
    ...
    public function Send_Mail(){  
        ...
        //print($comprobante['comprobante_id']);exit;
        /*crear pdf*/
        $this->create_pdf($comprobante_id);
        // Receiver email address
        $this->email->to($comprobante['cli_email']);
        $this->email->cc('hostalmarvinchincha@gmail.com');
...
//        echo $obj['contenido'];
    ...
    public function mombreCodigoDeTributo($codigoIgv)
    {
     ...
     public function comunicacionBaja() {
        ...
        $fecha_baja_desde = '';
        if (!empty($this->input->post('fecha_baja_desde'))) {
            $date = new DateTime($this->input->post('fecha_baja_desde'));
            $fecha_baja_desde = '"'.$date->format('Y-m-d').'"';

        }
        //print_r($fecha_de_emision);exit();
        $fecha_baja_hasta = '';
        if (!empty($this->input->post('fecha_baja_hasta'))) {
            $date = new DateTime($this->input->post('fecha_baja_hasta'));
            $fecha_baja_hasta = '"' . $date->format('Y-m-d') . '"';
        }      
        //echo $fecha_baja_desde.' '.$fecha_baja_hasta;exit;
        $data['comprobante'] = $this->comprobantes_model->select('', '', '', '', '', '', '', '', 1,FALSE,FALSE,'','','','',$fecha_baja_desde,$fecha_baja_hasta);
        $this->accesos_model->menuGeneral();
        $this->load->view('comprobantes/comunicacionBaja', $data);
        $this->load->view('templates/footer');
    }
    
    public function comunicacionBaja_1() {        
        $rsDatos = $this->comprobantes_model->getMainListBaja();
    ...
    public function estadoBaja($comprobante_id = '', $cliente_id = '') {
        ...
        $comprobanteAnulado = $this->db->from('comprobante_anulados')                                        
                                       ->where('comprobante_id', $comprobante_id)
                                       ->get()->row();                               
        ...
        if($comprobante['tipo_documento_id'] == 3){
            ...
            $fecha_de_baja = date("d-m-Y",strtotime($fecha_de_baja."- 1 days")); 
            $tipoBaja = "RC";
            $numeroBaja = "00".$comprobanteAnulado->numero;            
        } else {
            $tipoBaja = "RA";        
            $numeroBaja = $comprobanteAnulado->numero;
        }
        
        $date = new DateTime($fecha_de_baja);
        $fecha_de_baja = $date->format('Ymd');                                                
        //var_dump($comprobante);exit;
        //echo 'ftp://nwwotsiqds2z:Ckanyatno12%@mundosoftperu.com/MUNDOSOFTPERUSUNATBETA/sfs/'.$comprobante['empresa_ruc'].'/XML/'.$comprobante['empresa_ruc'].'-RA-' . $fecha_de_baja .'-'.$comprobanteAnulado->numero. '.xml';exit;
        $archivo =  fopen('ftp://nwwotsiqds2z:Ckanyatno12%@mundosoftperu.com/public_html/MUNDOSOFTPERUSUNAT/sfs/'.$comprobante['empresa_ruc'].'/XML/'.$comprobante['empresa_ruc'].'-'.$tipoBaja.'-' . $fecha_de_baja .'-'.$numeroBaja. '.xml','r');
        //$file = RUTA_API.'sfs/'.$comprobante['empresa_ruc'].'/XML/'.$comprobante['empresa_ruc'].'-RA-' . $fecha_de_baja .'-'.$comprobanteAnulado->numero. '.XML';        
...
//            '',
    ...
    public function get_xml($comprobante_id)
    {
        ...
        $archivoXML = "{$rsComprobante->empresa_ruc}-{$rsComprobante->codigo}-{$rsComprobante->serie}-{$rsComprobante->numero}.xml";
        //$rutaFirma = DISCO.':\xampp\htdocs'.CARPETA."/".SFS."/sunat_archivos/sfs/FIRMA/{$archivoXML}";
        $rutaFirma = APPPATH . "files_pdf/comprobantes/{$archivoXML}";
        //$rutaFirma = 'mundosoftperu.com/MUNDOSOFTPERUSUNAT/sfs/'.$rsComprobante->empresa_ruc."/XML/{$archivoXML}";
    ...
    public function exportarExcel()
    {
        ...
        foreach($resultComprobantes as $index => $item)
        {
            ...
            $objPHPExcel->getActiveSheet()
                        ...
                        ->setCellValue('N' . ($i), $rsPago->tipo_pago)
    ...
    public function getDatosXML()
    {
        ...
        ////DATOS ITEMS 
            ...
            foreach($items as $i){
                ...
                $datos_item['igv'] = (intval($i->codigo)>=20)?0:round($i->total-($i->total/1.18),2);
                $datos_item['base'] = (intval($i->codigo)>=20)?$i->total:round(($i->total/1.18),2);
                $datos_item['porcentaje_igv'] = 18;
                ...
                $datos_item['valor_icbper'] = $configuracion->monto_icbper;
                $datos_item['igv_icbper'] = $i->cantidad*$configuracion->monto_icbper;
    ...
    public function getDatosSunat()
    {
        ...
        if($opc == 0){
            ...
            foreach($items as $i){
                ...
                $datos_item['cantidad'] = $i->cantidad;
                $datos_item['descripcion'] = $i->descripcion;
                $datos_item['igv'] = ($configuracion->pu_igv==1)?round($i->total-($i->total/1.18),2):round($i->total*0.18,2);
                ...
                $datos_item['valor_unitario'] = ($configuracion->pu_igv==1)?round($i->importe/1.18,2):round($i->importe,2);
                $datos_item['precio_unitario'] = ($configuracion->pu_igv==1)?round($i->importe,2):round($i->importe*1.18,2);
